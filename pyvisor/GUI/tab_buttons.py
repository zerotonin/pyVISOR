...
class TabSimpleButtons(QWidget):
    ...
    def set_config(self, config):
        ...
        for key in ['keys', 'behavAssignment']:
            config[key] = BehavBinding.from_savable_dict_to_dict_of_objects(config[key])

        if config['selected_device'] not in self.input_device_names:
            QMessageBox.warning(self, 'Set config failed!',
                                f"Input device '{config['selected_device']}' not found.",
                                QMessageBox.Ok)
            return

        listOfSuperfluousBehav = self._get_assigned_buttons_of_undefined_behaviours(config)

        if len(listOfSuperfluousBehav) == 0:
            self.setButtonPreset(config)
            return

        self._handle_superfluous_behaviours(config, listOfSuperfluousBehav)
    ...
    def load_button_assignments(self, filename=None):
        ...
        self._handle_superfluous_behaviours(button_bindings, listOfSuperfluousBehav)

    def _handle_superfluous_behaviours(self, button_bindings, listOfSuperfluousBehav):
    ...
    def save_button_assignments(self, filename=None):
        ...
        button_binding_save_dict = self.create_button_binding_dict()

        if filename is None:
            filename = QFileDialog.getSaveFileName(self, 'Save Button Binding', HOME, initialFilter='*.json')
            filename = str(filename[0])
        if len(filename) == 0:
            return

        with open(filename, 'wt') as filehandler:
            json.dump(button_binding_save_dict, filehandler, indent=4)

    def create_button_binding_dict(self):
        d = {}
